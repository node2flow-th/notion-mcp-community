...
async function startHttp() {
  ...
  app.post('/mcp', async (req: any, res: any) => {
    ...
    if (qApiKey) process.env.NOTION_API_KEY = qApiKey;
 ...
 */
export default function createSmitheryServer(opts?: { config?: { NOTION_API_KEY?: string } }) {
  if (opts?.config?.NOTION_API_KEY) process.env.NOTION_API_KEY = opts.config.NOTION_API_KEY;
  const config = getConfig();
